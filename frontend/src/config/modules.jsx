...
export const STATUS_TONE = {
  Open: 'bad', 'In Progress': 'warn', Closed: 'good', Completed: 'good',
  Scheduled: 'neutral', Done: 'good', Cancelled: 'bad',
  Low: 'good', Medium: 'warn', High: 'bad',
  Draft: 'neutral', Submitted: 'warn', Approved: 'good', Active: 'good', Rejected: 'bad', Expired: 'bad',
  'Under Review': 'warn', Obsolete: 'bad',
  'On Track': 'good', Achieved: 'good', 'Not Achieved': 'bad', 'At Risk': 'warn'
};
...
export const MODULES = {
  ...
  hse_performance: {
    ...
    columns: [
      { key: 'date', label: 'Tanggal' },
      { key: '_gender', label: 'Tenaga Kerja (L/P)', render: (r) => `${r.male_workers ?? 0} / ${r.female_workers ?? 0}` },
      { key: 'working_hours', label: 'Jam Kerja/Hari' },
      { key: 'man_hours_today', label: 'Man-Hour Hari Ini', format: 'number' },
      { key: 'man_hours_cumulative', label: 'Man-Hour Kumulatif', format: 'number' },
    ...
    fields: [
      ...
      { key: 'working_hours', label: 'Jam Kerja Normal per Hari', type: 'number', default: 8, required: true },
      {
        key: '_man_hours_preview',
        label: 'Man-Hour Hari Ini (otomatis)',
        type: 'computed',
        compute: (v) => {
          const total = (Number(v.male_workers) || 0) + (Number(v.female_workers) || 0);
  ...
  permits: {
    ...
    columns: [
      ...
      { key: 'status', label: 'Status', badge: 'status' },
      { key: '_jsa', label: 'JSA', jsaColumn: true }
    ],
    fields: [
...
export const VIEW_META = {
  ...
  hse_performance: ['HSE Performance', 'Data harian: tenaga kerja hadir, Man-Hour otomatis, dan SR/FR/TRIR/LTIF kumulatif'],
  permits: ['Ijin Kerja', 'Work permit: hot work, confined space, working at height, dsb. — lengkap dengan JSA'],
